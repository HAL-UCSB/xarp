...
class GenerationSession:
    ...
    def __init__(self, params: GenerateParams,
                 config: OmegaConf, debug=False, frame_callback: Optional[Callable] = None,
        ...
        if self.input_video is not None:
            ...
            self.noise = latents * (1.0 - init_denoising_strength_scaled) + torch.randn(latents.shape,
                                                                                        device=self.noise.device,
                                                                                        dtype=self.noise.dtype,
                                                                                        generator=self.rnd) * init_denoising_strength_scaled
    ...
    def to(self, gpu):
        ...
        if isinstance(self.encode_vae_cache, list):
            self.encode_vae_cache = [cache.to(gpu, non_blocking=True) if cache is not None else None for cache in
                                     self.encode_vae_cache]
        if isinstance(self.decode_vae_cache, list):
            self.decode_vae_cache = [cache.to(gpu, non_blocking=True) if cache is not None else None for cache in
                                     self.decode_vae_cache]

        # move prompt embed stuff
        self.current_prompt_embeds = self.current_prompt_embeds.to(gpu,
                                                                   non_blocking=True) if self.current_prompt_embeds is not None else None
        ...
        self.interpolated_prompt_embeds = [embed.to(gpu, non_blocking=True) for embed in
                                           self.interpolated_prompt_embeds]
        self.gpu = gpu
    ...
    def get_clean_context_frames(self, models: Models):
        ...
        if self.params.keep_first_frame or (
                self.block_idx - 1) * models.pipeline.num_frame_per_block < current_kv_cache_num_frames:
            if current_kv_cache_num_frames == 1:
                clean_context_frames = clean_context_frames[:, :1]
            else:
                clean_context_frames = torch.cat(
                    (clean_context_frames[:, :1], clean_context_frames[:, 1:][:, -current_kv_cache_num_frames + 1:]),
                    dim=1)
        else:
            # print("reencoding first latent frame, block idx:")
            clean_context_frames = clean_context_frames[:, 1:][:, -current_kv_cache_num_frames + 1:]
            first_frame_latent = \
            encode_video_latent(models.vae_encoder, [None] * 55, resample_to=16, max_frames=81, video_path_or_url=None,
                                frames=self.frame_context_cache[0][0].half(), height=480, width=832, stream=False, )[
    ...
    def setup_start_frame(self, image: Image.Image, models: Models):
        ...
        latents = \
        encode_video_latent(models.vae_encoder, [None] * 55, resample_to=16, max_frames=81, video_path_or_url=None,
                            frames=tensors, height=480, width=832, stream=False)[0].transpose(0, 1)[None]
    ...
    def recompute_kv_cache(self, models: Models):
        ...
        models.pipeline._initialize_kv_cache(
            batch_size=clean_context_frames.shape[0], dtype=clean_context_frames.dtype,
    ...
    def generate_block_internal(self, models: Models):
        ...
        if self.params.webcam_mode:
            ...
            noisy_input = latents * (1.0 - denoising_strength_scaled) + torch.randn_like(
                latents) * denoising_strength_scaled
        else:
        ...
        for index, current_timestep in enumerate(self.denoising_step_list):
            ...
            if index < len(self.denoising_step_list) - 1:
                ...
                noisy_input = models.pipeline.scheduler.add_noise(
                    ...
                    torch.randn(*denoised_pred.flatten(0, 1).shape, generator=self.rnd, device=denoised_pred.device,
                                dtype=torch.bfloat16),
                    next_timestep * torch.ones([1 * models.pipeline.num_frame_per_block], device=self.noise.device,
                                               dtype=torch.long, )
        ...
        self.all_latents[:,
        self.current_start_frame:self.current_start_frame + models.pipeline.num_frame_per_block] = denoised_pred
...
async def ws_session(websocket: WebSocket, id: str, config: OmegaConf, models: Models):
    ...
    try:
        ...
        async def extract_frame(frames_future: asyncio.Future[torch.Tensor], idx: int, frame_id: str) -> bytes:
            ...
            await loop.run_in_executor(encode_pool,
                                       lambda: TF.to_pil_image(frames[0, idx], "RGB").save(io, format='JPEG',
                                                                                           quality=90))
        ...
        async for data in websocket.iter_bytes():
            ...
            if (image := frame.get("image")):
                await loop.run_in_executor(encode_pool, session.push_frame, image, frame.get("strength", None),
