...
from xarp.models import ChatMessage, Chat, Pose
from xarp.web_client.chat_ui import render_chat_messages, require_session_obj, uploaded_files_to_chat_files
from xarp.storage.local_file_system import SessionFileRepository, UserFileRepository
...
local_storage = pathlib.Path(*settings.local_storage.parts).absolute()
users = UserFileRepository(root=local_storage)
all_users = list(users.all())
user = st.selectbox('User ID', all_users, format_func=lambda u: u.user_id)

session = st.selectbox('Session', [None] + user.sessions, format_func=lambda s: str(s.ts) if s else 'New Session')

top = st.container()
bottom = st.container()

chat = session.chat if session else require_session_obj(chat_key, Chat)
        ...
        match xr_cmd.cmd:
            ...
            case 'head_pose':
                with bottom:
                    ...
                    pose.euler_angles = (st.slider('x angle', min_value=0, max_value=360),
            ...
            case 'see':
                if img_files := bottom.file_uploader('see', type=['jpg', 'jpeg', 'png'], accept_multiple_files=True):
                    ...
                    img_chat_files = uploaded_files_to_chat_files(img_files)
                    msg = ChatMessage.from_user('', files=img_chat_files)
                    chat.messages.append(msg)
...
elif ws_key in st.session_state:
    disconnect()
